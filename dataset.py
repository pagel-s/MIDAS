...
class ProcessedLigandPocketDataset(Dataset):
    def __init__(self, npz_path, center=True, transform=None, text_descriptions=None, text_embeddings_path=None):
        ...
        # Text handling: either descriptions (on-the-fly) or precomputed embeddings
    ...
    def collate_fn(batch):
        ...
        for prop in batch[0].keys():
            ...
            elif prop == 'text_description':
                out[prop] = [x[prop] for x in batch]
            elif prop == 'text_embedding':
                out[prop] = torch.stack([x[prop] for x in batch])
