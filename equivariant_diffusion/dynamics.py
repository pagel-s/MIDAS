...
class EGNNDynamics(nn.Module):
    ...
    def forward(self, xh_atoms, xh_residues, t, mask_atoms, mask_residues):
        ...
        # Optional FiLM-style text conditioning
        if hasattr(self, '_cond_film') and self._cond_film is not None:
            film_atoms = self._cond_film.get('atoms')
            film_res = self._cond_film.get('res')
            if film_atoms is not None and film_res is not None:
                gamma_a, beta_a = film_atoms  # film_atoms is a tuple (gamma, beta)
                gamma_r, beta_r = film_res    # film_res is a tuple (gamma, beta)
                # map batch-wise FiLM to node-wise using masks
                gamma_a_nodes = gamma_a[mask_atoms]
                beta_a_nodes = beta_a[mask_atoms]
                gamma_r_nodes = gamma_r[mask_residues]
                beta_r_nodes = beta_r[mask_residues]
                h_atoms = h_atoms * (1 + gamma_a_nodes) + beta_a_nodes
                h_residues = h_residues * (1 + gamma_r_nodes) + beta_r_nodes
