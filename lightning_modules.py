...
class LigandPocketDDPM(pl.LightningModule):
    def __init__(
        ...
        # Initialize text conditioning if needed
        if self.text_model_name is not None and self.text_embedder is None:
            if self.text_embeddings_path is not None:
                # Load embedding dimension from precomputed embeddings
                with np.load(self.text_embeddings_path, allow_pickle=True) as f:
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == 'fit':
            # Use precomputed embeddings if available, otherwise fall back to text descriptions
            if self.text_embeddings_path is not None:
                ...
                # Fallback to on-the-fly text descriptions
                train_texts, val_texts = None, None
                if self.text_csv is not None:
                    self._name_to_text = self._load_name_to_text(self.text_csv)
                    train_names = np.load(Path(self.datadir, 'train.npz'), allow_pickle=True)['names']
                    val_names = np.load(Path(self.datadir, 'val.npz'), allow_pickle=True)['names']
                    train_texts = [self._name_to_text.get(n, "") for n in train_names]
                    val_texts = [self._name_to_text.get(n, "") for n in val_names]

                self.train_dataset = ProcessedLigandPocketDataset(
                    Path(self.datadir, 'train.npz'), transform=self.data_transform,
                    text_descriptions=train_texts)
                self.val_dataset = ProcessedLigandPocketDataset(
                    Path(self.datadir, 'val.npz'), transform=self.data_transform,
                    text_descriptions=val_texts)
        elif stage == 'test':
            if self.text_embeddings_path is not None:
                ...
                test_texts = None
                if self.text_csv is not None:
                    if self._name_to_text is None:
                        self._name_to_text = self._load_name_to_text(self.text_csv)
                    test_names = np.load(Path(self.datadir, 'test.npz'), allow_pickle=True)['names']
                    test_texts = [self._name_to_text.get(n, "") for n in test_names]

                self.test_dataset = ProcessedLigandPocketDataset(
                    Path(self.datadir, 'test.npz'), transform=self.data_transform,
                    text_descriptions=test_texts)
        else:
    ...
    def forward(self, data):
        ...
        if self.text_model_name is not None:
            if 'text_embedding' in data:
                ...
                cond_vec = data['text_embedding'].to(self.device)
            ...
            if cond_vec is not None:
                ...
                self.ddpm.dynamics._cond_film = {
                    'atoms': (gamma_atoms, beta_atoms),
                    'res': (gamma_res, beta_res)
                }
        ...
        # Clear conditioning after use
        if hasattr(self.ddpm.dynamics, '_cond_film'):
            self.ddpm.dynamics._cond_film = None
    ...
    def generate_ligands(self, pdb_file, n_samples, pocket_ids=None,
                         ...
                         n_nodes_bias=0, n_nodes_min=0, text_description=None,
                         **kwargs):
        """
        ...
        # Optional text conditioning for inference
        reset_film = False
        if self.text_model_name is not None and text_description is not None:
            ...
            if isinstance(text_description, str):
                ...
                assert len(text_description) == n_samples, \
        ...
        # Clear conditioning after generation
        if reset_film and hasattr(self.ddpm.dynamics, '_cond_film'):
            self.ddpm.dynamics._cond_film = None
